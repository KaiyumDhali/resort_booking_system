...
@section('content')
    <main class="page-content">
        <div class="container">
            <div class="page-header">
                <h1 class="page-header__title">Products Complain List</h1>
            </div>
    ...
    <!--begin::Modal Complain Solve - Create App-->
    {{-- <div class="modal fade" id="complainSolveModal" tabindex="-1" aria-hidden="true">
        <div class="modal-dialog modal-dialog-centered mw-1000px ">
            <div class="modal-content ">
                <div class="modal-header">
                    <h5 class="modal-title" id="complainModalLabel">Complain Solve</h5>
                    <button type="button" class="close" data-dismiss="modal" aria-label="Close">
                        <span aria-hidden="true">&times;</span>
                    </button>
                </div>
                <form id="complainSolveModalForm" class="form fv-plugins-bootstrap5 fv-plugins-framework"
                        method="POST" action="{{ route('complaint_solve', ':complainSolvId') }}"
                    ...
                    <div class="modal-body scroll-y">
                        ...
                        <div class="d-flex flex-column mb-7 fv-row fv-plugins-icon-container">
                            <label class="required form-label">Complain Status</label>
    ...
    <script type="text/javascript">
        ...
        // function _complaintSolve(element) {
        //     let allData = $(element).data('all');            
        //     try {
        //         const formAction = $('#complainSolveModal').find('#complainSolveModalForm').attr('action');
        //         const complainSolvId = allData.id;
        //         const updatedFormAction = formAction.replace(':complainSolvId', complainSolvId);
        //         $('#complainSolveModal').find('#complainSolveModalForm').attr('action', updatedFormAction);
        //         $('#complainSolveModal').find('select[name="status"]').val(allData.status);
        //         $('#complainSolveModal').modal('show');
        //     } catch (error) {
        //         console.error('Error parsing JSON:', error);
        //     }
        // }
        function complaintSolve(element) {
            ...
            $.ajax({
                ...
                data: {
                    ...
                    status: 1 // Assuming '1' represents solved status
                },
                        ...
                        success: function(data) {
                            ...
                            if (data.length > 0) {
                                ...
                                $.each(data, function(key, value) {
                                    ...
                                    // Solve button
                                    let complain_solve = `<a href="#" class="btn btn-success" data-all='${alldata}' onclick='complaintSolve(this)'>Solve</a>`;
