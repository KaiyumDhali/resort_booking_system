        ...
        @foreach ($product_serials as $product_serial)
            @for ($i = 1; $i <= $product_serial_qty; $i++)
                <div class="row" style="text-align:center;">
                    <div class="col-12">
                        <div class="print"
                            style="font-size: 10px; margin-top: 30px; padding-left:10px; padding-right:10px">
                            {{ $product_serial->product_serial }}</div>
                        <div class=""
                            style="text-align: center; display: inline-block; margin-bottom:0; padding-bottom: 0;">
                            {!! DNS1D::getBarcodeHTML("$product_serial->product_serial", 'C128', 1, 20, 'black', false, 1) !!}
                        </div>
                    </div>
                </div>
            @endfor
