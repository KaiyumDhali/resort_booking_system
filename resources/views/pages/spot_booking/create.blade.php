...
<div class="shad-card mb-6">
    ...
    <div class="shad-body">
        <div class="row g-4" id="room_boxes">
            ...
            <div class="col-md-3">
                <div class="pkg-btn room-btn" data-id="{{ $room->id }}"
                     data-title="{{ $room->room_name }}"
                     data-price="{{ $room->price_per_night }}"
                     onclick="toggleRoom(this)">
                    <span class="spot-check d-none">✔</span>
        ...
        box.insertAdjacentHTML('beforeend', `
            <div class="col-4">
                <button type="button"
                    ...
                    data-id="${spot.id}" ${isBookedSpot ? 'disabled' : ''}>
                    <span class="spot-check d-none">✔</span>
...
function getServicesForSelectedSpots(){
    ...
    return allServices.filter(service => {
        ...
        // ✅ Global service হলে সব সময় show
        if(service.is_global == 1) return true;

        // ✅ না হলে pivot match check
...
function renderServices(){
    ...
    tbody.innerHTML = '';
 serviceTotal = 0;
    document.getElementById('serviceTotalDisplay').innerText = '0';
    document.getElementById('summaryService').innerText = '0';
    ...
    services.forEach(service => {
         const dynamicPrice = getServiceDynamicPrice(service);
          const isEditable = parseInt(service.editable_status) === 1;
...
function getServiceDynamicPrice(service){
    ...
    if(!service.prices || service.prices.length === 0){
        return parseFloat(service.price || 0); // fallback
    }

    const capacity = getSelectedPackageCapacity();

    if(capacity === 0){
        return parseFloat(service.price || 0); // package select না থাকলে fallback
    }
...
function loadAvailableRooms(dateStr){
    ...
    fetch(`/available-rooms?date=${dateStr}`)
        .then(res => res.json())
        .then(rooms => {
            roomBoxes.innerHTML = '';
...
function updateDue(){
    ...
    if(due <= 0){
        ...
        dueEl.classList.remove('text-danger-custom');
    } else {
        dueEl.classList.remove('paid');
        dueEl.classList.add('text-danger-custom');
    }
}
...
/* ================= AJAX CUSTOMER ADD ================= */
document.addEventListener('DOMContentLoaded', function(){
    const customerForm = document.getElementById('new_card_form');
    if(!customerForm) return;

    customerForm.addEventListener('submit', function(e){
        e.preventDefault();

        const btn = document.getElementById('new_card_submit');
        ...
        .then(data => {
            if(data.success){
                // Select2 তে নতুন option যোগ করো এবং auto select করো
                const newOption = new Option(data.account_name, data.id, true, true);
                $('#changeCustomer').append(newOption).trigger('change');

                // Modal বন্ধ করো
                bootstrap.Modal.getInstance(
                    document.getElementById('add_customer_modal')
                ).hide();
