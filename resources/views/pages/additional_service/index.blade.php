    ...
    <tr class="text-start fs-7 text-uppercase gs-0">
        ...
        <th class="text-end">Actions</th>
                        ...
                        <tbody class="fw-semibold text-gray-700">
                            ...
                            @else
                                @if($service->spots->count())
                                    @foreach($service->spots as $spot)
                                        <span class="badge badge-light-info">
                                            {{ $spot->title }}
                                        </span>
                                    @endforeach
                                @else
                                    <span class="text-muted">No Spot</span>
                                @endif
                            @endif
                        </td>
                     <td>
                        @if($service->is_frontend == 1 && $service->is_backend == 0)
                        <span class="badge badge-light-danger">
                            Frontend
                        </span>
                        @elseif($service->is_frontend == 0 && $service->is_backend == 1)
                        <span class="badge badge-light-info">
                            Backend
                        </span>

                        @elseif($service->is_frontend == 1 && $service->is_backend == 1)
