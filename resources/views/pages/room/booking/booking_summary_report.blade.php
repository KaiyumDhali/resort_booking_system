...
<div class="report-card">
    ...
    {{-- ── SUMMARY STRIP ── --}}
@php
    // ✅ INV বাদ দিয়ে শুধু amount calculate
    $filteredReport = $report->filter(fn($r) => !str_starts_with($r->booking_no, 'INV'));

    $totalBookings = $report->count(); // সব booking count
...
@forelse($report as $i => $r)
    @php
        ...
        $net   = $r->total_amount - $r->discount;  // ✅ INV তেও net দেখাবে
        $due   = $isINV ? 0 : ($net - $paid);
    @endphp
    <tr>
        <td>{{ $i + 1 }}</td>
        ...
        <td><span class="room-tag">{{ $r->room_numbers ?? '—' }}</span></td>
        ...
        <td class="text-end {{ $due > 0 ? 'text-red' : 'text-green' }}">
             @if($isINV)
                <span style="font-size:11px; font-weight:600; color:#6f42c1;">Paid</span>
            @else
            {{ (!$isINV && $due > 0) ? number_format($due, 2) : '—' }}
              @endif
        </td>
    </tr>
@empty
    <tr>
        <td colspan="13" class="text-center py-4 text-muted">No bookings found.</td>
    </tr>
@endforelse
</tbody>
<tfoot>
