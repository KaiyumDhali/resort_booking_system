<x-default-layout>
    ...
    <div id="kt_app_toolbar" class="app-toolbar py-3 py-lg-6">
        <div id="kt_app_toolbar_container" class="app-container d-flex flex-stack">
            <div class="page-title d-flex flex-column justify-content-center">
                ...
                <span class="text-muted fs-7">Update terms information</span>
            </div>
        ...
        <div id="kt_app_content_container" class="app-container">
            ...
            <div class="card card-flush">
                <div class="card-body py-5">
                    ...
                    <form action="{{ route('terms-conditions.update', $term->id) }}" method="POST">
                        @csrf
                        @method('PUT')
                        ...
                        <div class="d-flex">
                            <div class="mb-5 col-md-6 pe-2">
                                ...
                                <select name="term_type1" id="termFor" class="form-select form-select-solid">
                                    ...
                                    <option value="spot"
                                        {{ old('term_type1', $term->term_type1) === 'spot' ? 'selected' : '' }}>
                                        Zone
                                    </option>

                                    <option value="service"
                                        {{ old('term_type1', $term->term_type1) === 'service' ? 'selected' : '' }}>
                                        Additional Service
                                    </option>
                                    <option value="room"
                                        {{ old('term_type1', $term->term_type1) === 'room' ? 'selected' : '' }}>
                                        Room
                                    </option>

                                    {{-- common value (DB তে common থাকলে selected হবে) --}}
                                    <option value="common"
                                        {{ old('term_type1', $term->term_type1) === 'common' ? 'selected' : '' }}>
                                        Common
                                    </option>
                                </select>
                            </div>
                            ...
                            <div class="mb-5 col-md-6 ps-2">
                                ...
                                <select name="term_type" id="termType" class="form-select form-select-solid">
                                    ...
                                    <option value="included"
                                        {{ old('term_type', $term->term_type) === 'included' ? 'selected' : '' }}>
                                        Included
                                    </option>

                                    <option value="common"
                                        {{ old('term_type', $term->term_type) === 'common' ? 'selected' : '' }}>
                                        Common
                                    </option>
                                </select>
                            </div>
                        ...
                        <div class="mb-5 d-none" id="roomField">
                            ...
                            <select name="room_id" class="form-select form-select-solid">
                                <option value="">-- Select Room --</option>
                                @foreach ($rooms as $room)
                                    <option value="{{ $room->id }}"
                                        {{ old('room_id', $term->room_id) == $room->id ? 'selected' : '' }}>
                                        {{ $room->room_number }}
                                    </option>
                                @endforeach
                            </select>
                        ...
                        <div class="mb-5">
                            ...
                            <input type="text" name="term_title" class="form-control form-control-solid"
                                value="{{ old('term_title', $term->term_title) }}" placeholder="e.g. Entry Validity">
                        </div>
                        ...
                        <div class="mb-5">
                            ...
                            <select name="is_active" class="form-select form-select-solid">
                                <option value="1" {{ old('is_active', $term->is_active) == 1 ? 'selected' : '' }}>
                                    Active
                                </option>
                                <option value="0" {{ old('is_active', $term->is_active) == 0 ? 'selected' : '' }}>
                                    Disabled
                                </option>
                            </select>
                        </div>
    ...
    function clearHiddenFields() {
        if (document.getElementById('spotField').classList.contains('d-none')) {
            document.querySelector('select[name="spot_id"]').value = '';
        }
        if (document.getElementById('serviceField').classList.contains('d-none')) {
            document.querySelector('select[name="additional_service_id"]').value = '';
        }
