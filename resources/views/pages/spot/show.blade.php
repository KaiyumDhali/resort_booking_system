<x-default-layout>
    <div class="app-main flex-column flex-row-fluid" id="kt_app_main">
        <div class="d-flex flex-column flex-column-fluid">
            <!--begin::Toolbar-->
            <div id="kt_app_toolbar" class="app-toolbar py-3 py-lg-6">
                <div id="kt_app_toolbar_container" class="app-container d-flex flex-stack">
                    <div class="page-title d-flex flex-column justify-content-center flex-wrap me-3">
                        <h1 class="page-heading d-flex text-dark fw-bold fs-3 flex-column justify-content-center my-0">
                            Spot Details
                        </h1>
                    </div>
                    {{-- <div class="d-flex align-items-center gap-2 gap-lg-3">
                        <a href="{{ route('about_us.index') }}" class="btn btn-sm btn-light-primary">
                            <i class="ki-duotone ki-arrow-left fs-2"></i>Go Back
                        </a>
                    </div> --}}
                </div>
            </div>
            <!--end::Toolbar-->

            <!--begin::Content-->
            <div id="kt_app_content" class="app-content flex-column-fluid">
                <div id="kt_app_content_container" class="app-container">
                    <div class="card card-flush">
                        <div class="card-body py-5">
                            <div class="row">
                                <div class="col-md-6">
                                    <table class="table table-hover table-bordered">
    <tbody>
        <tr>
            <th class="w-25">Title</th>
            <td>{{ $spot->title }}</td>
        </tr>

        <tr>
            <th>Description</th>
            <td>{{ $spot->description }}</td>
        </tr>

        <tr>
            <th>Price</th>
            <td>{{ $spot->price }}</td>
        </tr>

        <tr>
            <th>Status</th>
            <td>{{ $spot->status == 1 ? 'Active' : 'Disable' }}</td>
        </tr>

        @if($spot->facilities->count())
        <tr>
            <th>Facilities</th>
            <td>
                <ul class="mb-0 ps-3">
                    @foreach($spot->facilities as $facility)
                        <li>{{ $facility->facility }}</li>
                    @endforeach
                </ul>
            </td>
        </tr>
        @endif
    </tbody>
</table>

                                </div>
                                <div class="col-md-6">
                                    {{-- @if ($spot->image)
                                        <div class="form-group pb-2">
                                            <h5 class="pb-3">Spot Image:</h5>
                                            <img src="{{ asset(Storage::url($spot->image)) }}" height="220"
                                                width="320" alt="About Us Image" />
                                        </div>
                                    @endif --}}


                                    <div class="col-md-12">
                                        @if ($spot->spot_detail->count())
                                            <div class="form-group pb-2">
                                                <h5 class="pb-3">Spot Detail Images:</h5>
                                                <div class="d-flex flex-wrap gap-2">
                                                    @foreach ($spot->spot_detail as $detail)
                                                        @if ($detail->image_path)
                                                            <img src="{{ asset(Storage::url($detail->image_path)) }}"
                                                                height="220" width="320" alt="Spot Detail Image"
                                                                class="img-thumbnail" />
                                                        @endif
                                                    @endforeach
                                                </div>
                                            </div>
                                        @endif
                                    </div>


                                </div>
                            </div>

                            <div class="d-flex justify-content-end">
                                <!--begin::Button-->

                                <a class="btn btn-sm btn-primary me-2" href="{{ route('spots.index') }}"
                                    id="kt_ecommerce_add_product_cancel">Back</a>

                                <a class="btn btn-sm btn-info me-2" href="{{ route('spots.edit', $spot->id) }}"
                                    id="kt_ecommerce_add_product_cancel">Edit</a>

                                <form action="{{ route('spots.destroy', $spot->id) }}" method="POST">
                                    @csrf
                                    @method('DELETE')
                                    <button type="submit" class="btn btn-sm btn-danger px-4"
                                        onclick="return confirm('Are you sure you want to delete this entry?')">
                                        Delete
                                    </button>
                                </form>

                                <!--end::Button-->
                            </div>


                        </div>
                    </div>
                </div>
            </div>
            <!--end::Content-->
        </div>
    </div>
</x-default-layout>
