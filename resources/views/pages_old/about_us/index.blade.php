                    ...
                    <div class="card-title">
                        <div class="d-flex align-items-center position-relative my-1">
                            ...
                            <input type="text" data-kt-ecommerce-order-filter="search"
                                ...
                                placeholder="Search AboutUs" />
                        </div>
                    </div>
                    <div class="card-toolbar flex-row-fluid justify-content-end gap-5">
                        <a href="{{ route('about_us.create') }}" class="btn btn-sm btn-primary">Add AboutUs</a>
                    </div>
                </div>
                ...
                <div class="card-body pt-0">
                    <table class="table table-striped table-bordered align-middle table-row-dashed fs-7 gy-5 mb-0">
                        ...
                        <tbody class="fw-semibold text-gray-700">
                            @forelse($about_us as $aboutUs)
                                <tr>
                                    <td>{{ $loop->iteration }}</td>
                                    <td>{{ $aboutUs->title }}</td>
                                    <td><img src="{{ asset(Storage::url($aboutUs->image)) }}" height="75"
                                            width="120" alt="Not Found" />
                                    </td>
                                    <td>{{ $aboutUs->image_size }}</td>
                                    <td>
                                        <span
                                            class="badge badge-light-{{ $aboutUs->status == 1 ? 'success' : 'secondary' }}">
                                            {{ $aboutUs->status == 1 ? 'Active' : 'Disable' }}
                                        </span>
                                    </td>
                                    ...
                                    {{-- <td class="text-end">
                                        <div class="btn-group btn-group-sm">
                                            <a href="{{ route('about_us.edit', $aboutUs->id) }}"
                                                class="btn btn-sm btn-info p-2">
                                                <i class="fa fa-pencil-alt"></i>
                                            </a>
                                            <a href="{{ route('about_us.show', $aboutUs->id) }}"
                                                class="btn btn-sm btn-primary p-2">
                                                <i class="fa fa-eye"></i>
                                            </a>
                                            <form onclick="return confirm('Are you sure?')" class="d-inline"
                                                action="{{ route('about_us.destroy', $aboutUs->id) }}" method="POST">
                                                @csrf
                                                @method('delete')
                                    ...
                                    <td class="text-end">
                                        <div class="btn-group" role="group">
                                            ...
                                            {{-- Delete Button --}}
                                            {{-- <form action="{{ route('about_us.destroy', $aboutUs->id) }}" method="POST"
                                                class="d-inline"
