                ...
                <div class="card shadow px-10 pb-8 ms-0 ms-md-5">
                    ...
                    <div class="form-group">
                        ...
                        <input type="hidden" class="form-control form-control-sm form-control-solid" id="voucher_no"
                            name="voucher_no" value="{{ $customerFinance->voucher_no }}" />
                        <input type="hidden" class="form-control form-control-sm form-control-solid"
                            id="delivery_challan_no" name="delivery_challan_no"
                            value="{{ $customerFinance->delivery_challan_no }}" />
                    </div>
                    <div class="form-group">
                        ...
                        <select id="changeAccountsName" class="form-control form-control-sm" name="receive_account"
                            ...
                            @foreach ($toAccounts as $toAccount)
                                <option value="{{ $toAccount->id }}"
                                    @if ($customerPayment && $customerPayment->to_acc_name && $toAccount->account_name == $customerPayment->to_acc_name) selected @endif>
...
<script type="text/javascript">
    ...
    $('#addProduct').on('click', function() {
        ...
        // SweetAlert2 check for sales_price
        if (!sales_price || sales_price <= 0) {
            Swal.fire({
                icon: 'error',
                title: 'Invalid Sales Price.',
                text: 'Please enter a valid sales price.',
            });
            return;
        }
    ...
    function loadProductsFromData() {
        getData.forEach(product => {
            const stock_out_total_amount = parseFloat(product.stock_out_total_amount) ||
            0; // Convert to number, default to 0 if NaN
            const discount = parseFloat(product.stock_out_discount) ||
            0; // Convert to number, default to 0 if NaN
            addProductRow({
    ...
    $('#changeCustomer').on('change', function() {
        var acName = $(this).val();
        if (acName) {
            var url = '{{ route('productCustomerDetails', ['id' => ':ac_name']) }}'.replace(':ac_name',
            acName);
            console.log(url);
            $.ajax({
                url: url,
